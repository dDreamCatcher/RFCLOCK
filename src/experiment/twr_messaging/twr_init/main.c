...
extern void pin_event_handler(nrf_drv_gpiote_pin_t pin, nrf_gpiote_polarity_t action);
...
int main(void)
{
  ...
  * As this example only handles one incoming frame with always the same delay and timeout, those values can be set here once for all. */
  ...
  dwt_setrxtimeout(1000000); // Maximum value timeout with DW1000 is 65ms 
...
uint32 INITIAL_system_time_counter;   
INITIAL_system_time_counter= dwt_readsystimestamphi32();

printf("INITIAL_system_time_counter: %u\r\n", INITIAL_system_time_counter);
